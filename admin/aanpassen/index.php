<?php
    ...
    # Meldingen bij errors
    if (isset($_GET['blok'])) {
		$blok = $_GET['blok'];

        if ($blok == 'empty') {
            echo '<div class="alert alert-danger text-center" role="alert">U heeft niet alle velden ingevuld!</div>';
        } else if ($blok == 'number') {
            echo '<div class="alert alert-danger text-center" role="alert">U heeft geen geldige cijfers ingevoerd!</div>';
        } else if ($blok == 'char') {
            echo '<div class="alert alert-danger text-center" role="alert">U heeft geen tekstwaardes ingevoerd!</div>';
        }
    }
            ...
            <form method="POST" action="./aanpassen.php">
                ...
                <div class="form-row mt-4">
                    <div class="form-group col-md-6">
                        ...
                        <input type="text" class="form-control" name="start_tijd" id="start_tijd" value="<?= $row['start_tijd']?>">
                    </div>
                    <div class="form-group col-md-6">
                        <label for="eind_tijd">Eind tijd</label>
                        <input type="text" class="form-control" name="eind_tijd" id="eind_tijd" value="<?= $row['eind_tijd']?>">
                    </div>
                </div>
